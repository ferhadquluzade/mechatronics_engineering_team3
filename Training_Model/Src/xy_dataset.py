...
class XYDataset_static(torch.utils.data.Dataset):
    def _parse(self, path):
        basename = os.path.basename(path)
        items = basename.split('_')
        x = items[0]
    ...
    def __init__(self, data_dir, categories, transform=None, random_hflip=False):
    ...
    def get_img_info(self, data_dir):
        ...
        for path in data_dir:
            path = path.replace("\\", "/")
            x, y = self._parse(path)
            data_info.append((path, int(0), int(x), int(y)))
        return data_info
    
    def __getitem__(self, index):
        path_img, category_idx, x, y = self.data_info[index]
        ...
        return image, category_idx, torch.Tensor([x, y])
